	...
	FileList.prototype = {
        ...
        do_tags:function(files) {
            ...
            if($('#tags-value') && $("#tags-value").length == 0) {
                $('#selectedActionsList').append('<input name="tags-value" id="tags-value" /><button id="tag-action">Tag</button>');
            }
            $("#tag-action").on("click touch", function () {
                var tagsValue = $("#tags-value").val().split(',');
                var trimTagsValue = [];
                $.each(tagsValue, function (index, tagValue) {
                    var r = $.trim(tagValue);
                    trimTagsValue.push(r);
                });
                ...
                $.each (files, function(index, file) {
                    ...
                    console.log(path);
                    $.ajax({
                        'url': '/index.php/apps/resttagapi/api/v1/restapi/'+path,
                        'method': 'PUT',
                        'data': tags,
